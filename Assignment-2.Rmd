...
ggplot(final_results_L) +
  ...
  labs(title = "Distribution of LDA and QDA Training and Test Error Rates -- Linear",
       ...
       legend = "Model")
```
We can observe that the LDA test error is slightly lower than the QDA test error, consistent with the notion that LDA models are prefered in cases on lienar Bayes deicison boundaries.
...
```{r, message=FALSE}
##Crteate functions for data and different number of sims
...
ggplot(final_results_NL) +
  ...
  labs(title = "Distribution of LDA and QDA Training and Test Error Rates -- Non-Linear",
       ...
       legend = "Model")
```
The graph supports the notion that QDA models are preferable in cases on non-linear Bayes decision boundaries.

## Question 4.
###A. 
As the sample size increases, we generally expect QDA to be preferable in large n cases as the test error rate will improve compared to LDA. Even though QDA models are prone to higher variance, that concern in minimized with larger sample sizes.
...
cols <- c("LDA Test Error (1000)" = "aquamarine3", "QDA Test Error (1000)" = "aquamarine4",
          ...
          "LDA Test Error (100000)" = "green", "QDA Test Error (10000)" = "green4")
ggplot() +
...
We can observe above that QDA performs better as the sample size increases. In the alrgest sample size above, the LDA model (light green) has a larger error rate than the QDA model (dark green).
...
The KNN model with 7 nearest neighbors performs the best, as indicated by its higher AOC score. By best, I mean that it has the highest AOC value (compare true positives against false positives) and thus has the strongest predictive power.
