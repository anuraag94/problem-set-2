...
x <- ggplot(data = bg_grid, mapping = aes(X1, X2, z = Success))
...
###Error Rates
  - Logistic regression model: `r logit_error`
  - Linear discriminant model: `r error_lda_test`
  - Quadratic discriminant model: `r error_qda_test`
...
library(pROC)
#Logit
mh_logit_train <- predict(mh_logit, 
                newdata = train_mh)
mh_logit_train$class
test_mh

#LDA
lda_roc <- roc(as.numeric(train_mh$vote96), as.numeric(mh_lda_train$class))
lda_auc <- auc(lda_roc)

#QDA
qda_roc <- roc(as.numeric(train_mh$vote96), as.numeric(mh_qda_train$class))
qda_auc <- auc(qda_roc)
